...
class ExpandDialog():
    ...
    def key_pressed(widget, what, event):
        if event.keyval == 65293: # Return
            ...
            widget.exit = False
            widget.done = widget.editor.do_expand_with_abbreviation(widget.done, '')
            widget.quit()
        else:
            return False
    ...
    def update(self, entry):
        ...
        self.done = self.editor.do_expand_with_abbreviation(self.done, self.abbreviation)
...
def main(editor, window, text=""):
    ...
    expand_dialog = ExpandDialog(editor, xo + xb, yo + yb, text)
    expand_dialog.main()
    ...
    return expand_dialog.done and expand_dialog.exit, expand_dialog.abbreviation
